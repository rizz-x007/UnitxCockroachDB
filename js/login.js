...
function renderTurnstile() {
    ...
    // Safe teardown: Removes turnstile container entirely if signup is not active
    if (!isSignupVisible) {
    ...
    // Prevents redundant renders by purging old container blocks completely
    if (turnstileWidgetId !== null) {
        turnstile.remove(turnstileWidgetId);
        turnstileWidgetId = null;
    }
    const existingContainer = document.getElementById("turnstile-container");
    if (existingContainer) existingContainer.remove();

    // Re-creates pristine container node
    const container = document.createElement("div");
    container.id = "turnstile-container";
    container.style.marginBottom = "1rem";

    // Safe insertion hierarchy
