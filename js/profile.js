...
async function initializeProfile() {
    try {
        ...
        let response = await window.authFetch("/api/profile");
        ...
        if (!result.profile || !result.profile.id) {
            const saveRes = await window.authFetch("/api/profile/save", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ college_name: "", location_name: "", address: "" })
            });
            if (saveRes.ok) {
                const retry = await window.authFetch("/api/profile");
...
async function loadProfileData(accountData) {
    ...
    const domLocationInput = document.getElementById("location");

    if (collegeInput)  collegeInput.value  = profile.college_name  || "";
    if (domLocationInput) domLocationInput.value = profile.location_name || "";
...
if (verificationSummary) {
    verificationSummary.addEventListener("click", async (e) => {
        ...
        if (deleteBtn) {
            ...
            try {
                const res = await window.authFetch(`/api/profile/verify/${type}`, { method: "DELETE" });
...
if (profileForm) {
    profileForm.addEventListener("submit", async (e) => {
        ...
        try {
            const res = await window.authFetch("/api/profile/save", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
...
if (avatarUpload) {
    avatarUpload.addEventListener("change", async () => {
        ...
        try {
            const res = await window.authFetch("/api/profile/avatar", {
                method: "POST",
                body: formData
            });
            const result = await res.json().catch(() => ({ success: false }));
...
if (verifyStudentBtn) {
    verifyStudentBtn.addEventListener("click", async () => {
        ...
        try {
            const res = await window.authFetch("/api/profile/verify/student", {
                method: "POST",
                body: formData
            });
...
if (sellerVerifyBtn) {
    sellerVerifyBtn.addEventListener("click", async () => {
        ...
        try {
            const res = await window.authFetch("/api/profile/verify/seller", {
                method: "POST",
                body: formData
            });
            const result = await res.json().catch(() => ({ success: false }));
...
document.addEventListener("keydown", async (e) => {
    ...
    if (!token) return;
    try {
        const res    = await window.authFetch("/api/admin/check");
...
async function loadListings() {
    ...
    try {
        const res = await window.authFetch("/api/profile/my-listings");
        const result = await res.json().catch(() => ({ success: false }));
...
if (listingContainer) {
    listingContainer.addEventListener("click", async (e) => {
        ...
        try {
            const res = await window.authFetch(`/api/products/${id}`, { method: "DELETE" });
            const result = await res.json().catch(() => ({ success: false }));
